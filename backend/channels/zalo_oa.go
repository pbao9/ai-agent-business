...
func (z *ZaloOAAdapter) refreshToken(ctx context.Context) error {
	...
	log.Printf("[zalo] refreshing token app_id=%s oa_id=%s refresh_token=%s",
		z.creds.AppID, z.creds.OAId, maskToken(z.creds.RefreshToken))

	req, err := http.NewRequestWithContext(ctx, "POST", zaloOAuthURL, strings.NewReader(form.Encode()))
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("zalo token refresh read body failed: %w", err)
	}
	log.Printf("[zalo] refresh response status=%d body=%s", resp.StatusCode, sanitizeZaloTokenBody(body))
	...
	if result.AccessToken == "" {
		return fmt.Errorf("zalo token refresh returned empty access_token")
	}
	if result.RefreshToken == "" {
		return fmt.Errorf("zalo token refresh returned empty refresh_token")
	}
	...
	log.Printf("[zalo] refresh succeeded app_id=%s oa_id=%s access_token=%s refresh_token=%s",
		z.creds.AppID, z.creds.OAId, maskToken(result.AccessToken), maskToken(result.RefreshToken))
...
func sanitizeZaloTokenBody(body []byte) string {
	var payload map[string]interface{}
	if err := json.Unmarshal(body, &payload); err != nil {
		return string(body)
	}
	if accessToken, ok := payload["access_token"].(string); ok {
		payload["access_token"] = maskToken(accessToken)
	}
	if refreshToken, ok := payload["refresh_token"].(string); ok {
		payload["refresh_token"] = maskToken(refreshToken)
	}
