...
func (s *SyncEngine) SyncChannel(ctx context.Context, channel models.Channel) error {
	...
	if zaloAdapter, ok := adapter.(*channels.ZaloOAAdapter); ok {
		...
		zaloAdapter.SetTokenRefreshCallback(func(newAccess, newRefresh string) error {
			...
			if err := db.DB.First(&ch, "id = ?", chID).Error; err != nil {
				return fmt.Errorf("load channel %s before token persist: %w", chID, err)
			}
			oldCreds, err := pkg.Decrypt(ch.CredentialsEncrypted, encKey)
			if err != nil {
				return fmt.Errorf("decrypt channel %s credentials for token persist: %w", chID, err)
			}
			var credsMap map[string]interface{}
			if err := json.Unmarshal(oldCreds, &credsMap); err != nil {
				return fmt.Errorf("unmarshal channel %s credentials for token persist: %w", chID, err)
			}
			log.Printf("[sync] persisting refreshed Zalo tokens for channel %s access_token=%s refresh_token=%s",
				chID, maskSyncToken(newAccess), maskSyncToken(newRefresh))
			...
			if err != nil {
				return fmt.Errorf("encrypt refreshed tokens for channel %s: %w", chID, err)
			}
			result := db.DB.Model(&ch).Updates(map[string]interface{}{
				"credentials_encrypted": encrypted,
				"updated_at":            time.Now(),
			})
			if result.Error != nil {
				return fmt.Errorf("update channel %s refreshed tokens: %w", chID, result.Error)
			}
			if result.RowsAffected == 0 {
				return fmt.Errorf("update channel %s refreshed tokens affected 0 rows", chID)
			}
